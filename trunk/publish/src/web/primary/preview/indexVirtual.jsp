...
<s:if test="#session.virtualPublisher != null">
	...
	<s:form theme="simple" name="approveDocumentForm" action="ApproveVirtualDocument" method="post" validate="true">	
    	<s:token />		
		<s:hidden name="id" />
		<s:hidden name="document.id" />
		<s:hidden name="document.documentname" />
		<s:submit key="button.approve" cssClass="fbtn"/>
		<s:submit action="ListVirtualDocumentPreProcess" key="button.cancel" onclick="form.onsubmit=null" cssClass="fbtn"/>		
	</s:form>
