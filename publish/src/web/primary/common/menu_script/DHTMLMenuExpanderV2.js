...
{hideALL(); showSelect();setSTYLES(); attachEventhandlers();}
...
for (a=0; a<TABLECol.length; a++)
	...
	THCol=TABLECol.item(a).getElementsByTagName("TH");
	for (b=0; b<THCol.length; b++)
		{THCol.item(b).setAttribute("ECState",1);}
...
if (TABLECol!=null)
	...
	for (z=0; z<TABLECol.length; z++)
		{
		if (TABLECol.item(z).id!="subTable"){
...
if(THElement!=null)
	...
	//showSingle();
	showClicked(THElement);
	}
else{hideALL();}
}

function showClicked(THElement)
{
var thisTRCol = THElement.parentNode.parentNode.parentNode.getElementsByTagName("TR");
	for (var k = 0; k < thisTRCol.length;k++){
		thisTRCol[k].style.display="block";
		var thisTD = thisTRCol[k].getElementsByTagName("TD");
		for (var m=0; m < thisTD.length;m++){
			thisTD[m].style.display="block";
		}
	}
}
...
mainTABLEElement=document.getElementById("mainTable");
TABLECol=mainTABLEElement.getElementsByTagName("TABLE");
var actionHref = document.location.pathname;
var submenu;
if (TABLECol!=null)
	{
	for (z=0; z<TABLECol.length; z++)
		...
		TRCol=TABLECol.item(z).getElementsByTagName("TR");
		THCol=TABLECol.item(z).getElementsByTagName("TH");
		ACol=TABLECol.item(z).getElementsByTagName("a");
		InputC=TABLECol.item(z).getElementsByTagName("input");
		// Grab ECState and expand or collapse branch
		
		   for (var y=0; y<ACol.length; y++){
			var Ahref=ACol.item(y).getAttribute("href");
			var ActionLink ="dummy";
			if (InputC.length>0){
				var ActionLink=InputC.item(y).getAttribute("name");
			}
			if ( actionHref.match(Ahref) || actionHref.match(ActionLink)
			  || Ahref.match(actionHref) || ActionLink.match(actionHref) ){
			  	
				THCol[0].setAttribute("ECState",1);
				ACol[y].parentNode.setAttribute("class","select");
				ACol[y].parentNode.setAttribute("className","select");
			/*	ACol[y].parentNode.parentNode.style.backgroundColor='white';
				ACol[y].parentNode.parentNode.style.color='black'; */
				submenu = ACol[y].parentNode.parentNode.parentNode;
				
				var thisTRCol = submenu.getElementsByTagName("TR");
				for (var k = 0; k < thisTRCol.length;k++){
					thisTRCol[k].style.display="block";
					var thisTD = thisTRCol[k].getElementsByTagName("TD");
					for (var m=0; m < thisTD.length;m++){
				...
				THCol[0].setAttribute("ECState",0);
			}
		   }
