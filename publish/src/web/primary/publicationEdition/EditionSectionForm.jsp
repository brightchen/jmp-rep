...
<s:form name="editionSectionForm" action="AddEditionSectionInput" method="post" validate="true" enctype="multipart/form-data">
	<fieldset>
		...
		<p>
            <s:select key="selectedPublicationName" list="publicationNames" onchange="document.editionSectionForm.submit()" required="true"/>
        </p>

        <s:select key="selectedEditionName" list="publicationEditionNames" required="true"/>
		<!-- s:textfield key="document.documentname" required="true" / -->		
		<s:select key="documentStyle" label="Style" list="styles" required="true"/>
		...
		<s:select key="selectedSectionName" list="publicationSectionNames" required="true" />
	...
	<div class="buttonrow">
		<s:submit action="AddEditionSection" key="button.save" cssClass="fbtn" />	
