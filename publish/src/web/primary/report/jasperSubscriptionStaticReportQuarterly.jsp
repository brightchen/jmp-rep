...
<s:form name="subscriptionReportStaticForm" action="SubscriptionStaticQuarterlyReport" method="post" validate="true">
	<fieldset>
   	<legend><s:text name="subscriptionQuarterlyreport.legend"/></legend>
	<s:token />
	
	<s:hidden key="reportStyle" value="Quarterly" />

	<s:select key="publicationName" list="publicationNames" required="true" />
	<s:select key="startQuarter" list="quarters" label="Start Quarter" required="true" cssStyle="width:60px"/>
	<s:select key="startYear" list="years" label="Start Year" required="true" cssStyle="width:60px"/>
	<s:select key="endQuarter" list="quarters" label="End Quarter" required="true" cssStyle="width:60px"/>
